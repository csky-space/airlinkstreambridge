...
ConfigurationClient::ConfigurationClient(std::string_view configurationServerUrl, std::string_view login, std::string_view password)
	: session(configurationServerUrl.data(), 443), apiURI(std::string(configurationServerUrl)), hostUrl(apiURI.getHost()), login(login), password(password) {
	Poco::Net::initializeSSL();
	Poco::SharedPtr<Poco::Net::InvalidCertificateHandler> pCertHandler = new Poco::Net::AcceptCertificateHandler(true);
	pContext =
...
WebrtcConfiguration ConfigurationClient::getConfiguration() const { return configuration; }
...
void ConfigurationClient::requestToConfig() {
	...
	for (const auto &server : replyBody["turnServers"]) {
		configuration.turnServersConfigs.push_back({server["url"], server["username"], server["credential"]});
	}
